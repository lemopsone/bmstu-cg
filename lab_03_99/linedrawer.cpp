...
QList<Pixel> LineDrawer::DDA(QPoint start, QPoint end, QColor color) {
  ...
  if (diff.isNull()) {
    ...
    length = std::max(qAbs(diff.x()), qAbs(diff.y()));
    ...
    for (int i = 0; i <= length; i++) {
      points << Pixel{.point = QPoint(std::round(x), std::round(y)),
                      .color = color};
      x += dX;
      y += dY;
    }
...
QList<Pixel> LineDrawer::Bresenham(QPoint start, QPoint end, QColor color) {
  ...
  if (diff.isNull()) {
    ...
    while (x != end.x() || y != end.y()) {
      ...
      if (e >= 0) {
        (swapped) ? x += signX : y += signY;
        e -= 1;
      }
      if (e <= 0) {
        (swapped) ? y += signY : x += signX;
        e += tan;
      }
    }
  }
...
QList<Pixel> LineDrawer::BresenhamSmooth(QPoint start, QPoint end,
  ...
  while (x != end.x() || y != end.y()) {
    points << Pixel{QPoint(x, y),
                    gradient.at(std::max(std::round(e) - 1, 0.0))};
    if (e < threshold) {
      (swapped) ? y += signY : x += signX;
      e += tan;
    } else {
...
QList<Pixel> LineDrawer::Wu(QPoint start, QPoint end, QColor foreground,
                            QColor background) {
  QList<Pixel> points;
  ...
  for (int x = rangeStartX; x < rangeEndX; x++) {
    if (steep) {
      ...
      points << Pixel{QPoint(x + 1, (int)y), gradient.at(intensity)};
      points << Pixel{QPoint(x + 1, (int)y + 1), gradient.at(intensity)};
      y += tan;
    }
  }
...
int sign(qreal value) { return (value > 0) ? 1 : (value < 0) ? -1 : 0; }
...
qreal getStep(int a1, int a2, const int I) { return (a2 - a1) / I; }
