...
SceneLine::SceneLine(QLine line, LineType type, QColor color, CoordinateScene *scene_)
    : SceneObject(scene_)
{
    this->line_ = line;
    this->type_ = type;
...
void SceneLine::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    case BRESENHAM_SMOOTH:
        pixels = LineDrawer::BresenhamSmooth(line_.p1(), line_.p2(), color_, scene->getColor());
        break;
    case WU:
