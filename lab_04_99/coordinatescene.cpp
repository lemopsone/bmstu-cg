...
CoordinateScene::CoordinateScene(QRectF windowRect, QRectF planeRect,
                                 bool isVisible, QColor color, QObject *parent)
...
void CoordinateScene::setCX(double c) { this->cX = c; }

void CoordinateScene::setCY(double c) { this->cY = c; }

double CoordinateScene::getCX(void) { return this->cX; }

double CoordinateScene::getCY(void) { return this->cY; }
...
bool CoordinateScene::gridIsVisible(void) { return gridVisible; }

QColor CoordinateScene::getColor() const { return color; }
...
void CoordinateScene::addEllipse(EllipseData data) {
  addItem(new SceneEllipse(data, this));
  update();
}

void CoordinateScene::addCircle(CircleData data) {
  addItem(new SceneCircle(data, this));
  update();
}

void CoordinateScene::addCircleSpectre(CircleData data, qreal step,
                                       qsizetype count) {
  for (qreal i = 0; i < count; i++) {
    CircleData tempData = data;
    tempData.radius += i * step;
    addCircle(tempData);
  }
  update();
}

void CoordinateScene::addEllipseSpectre(EllipseData data, qreal step,
                                        qsizetype count) {
  for (qreal i = 0; i < count; i++) {
    EllipseData tempData = data;
    tempData.a += i * step;
    tempData.b += i * step;
    addEllipse(tempData);
  }
...
void CoordinateScene::setPlane(double x, double y, double w, double h,
                               bool update) {
...
void CoordinateScene::setPlane(QPointF topLeft, double w, double h,
                               bool update) {
...
QRectF CoordinateScene::getPlane(void) { return this->plane; }

void CoordinateScene::setGraphicsWindow(double x, double y, double w, double h,
                                        bool update) {
...
QRectF CoordinateScene::getGraphicsWindow(void) { return this->graphicsWindow; }
...
QPointF CoordinateScene::toPlaneCoords(QPointF windowCoords) {
  ...
  if (this->cX == 0 || this->cY == 0) {
    throw new std::runtime_error(
        "Коэффициент равен нулю, невозможно перевести координаты");
  }
...
void CoordinateScene::drawBackground(QPainter *painter, const QRectF &rect) {
  ...
  //     QPointF bottom = this->toWindowCoords(QPointF(x,
  //     this->plane.bottom())); painter->drawLine(top, bottom);
  // }
...
void CoordinateScene::drawPoint(QPainter *painter, QPointF point,
  ...
  if (!text.isEmpty()) {
    this->drawText(painter, convertedPoint - QPointF(rx, -rx),
                   QSizeF(2 * rx, 2 * rx), text);
  }
}

void CoordinateScene::drawText(QPainter *painter, QPointF startingPos,
                               QString text) {
...
void CoordinateScene::drawText(QPainter *painter, QPointF startingPos,
  ...
  painter->drawText(QRectF(QPointF(0, 0), size),
...
QTransform CoordinateScene::getTransform() const { return this->transform; }
...
bool CoordinateScene::getAutoScale() { return this->autoScale; }

void CoordinateScene::autoScaleContents() {
  if (this->items().size() == 0) {
    this->setPlane(QPointF(-this->originalSize.width() / 2,
