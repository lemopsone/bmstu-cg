...
void CoordinateScene::drawEllipse(QPainter *painter, MyEllipseF ellipse)
{
    qsizetype n = 100;
    QPolygonF polygon;
    ...
    for (qsizetype i = 0; i < n; i++) {
        QPointF point;
        point.setX(
            center.x() + (a.x() - center.x()) * std::cos(i * 2 * M_PI / n) +
                (b.x() - center.x()) * std::sin(i * 2 * M_PI / n)
            );
        point.setY(
            center.y() + (a.y() - center.y()) * std::cos(i * 2 * M_PI / n) +
            (b.y() - center.y()) * std::sin(i * 2 * M_PI / n)
            );
        point = this->toWindowCoords(point);
        polygon << point;
    }
    painter->drawPolygon(polygon);
    this->update();
    // qDebug() << "Drawing ELLIPSE: " << ellipse.center() << ellipse.a() << ellipse.b();
}

void CoordinateScene::drawTrapezium(QPainter *painter, MyTrapeziumF trapezium)
{
    Q_ASSERT(trapezium.isValid());

    QList<QPointF> points = trapezium.getPoints();
    // Приведение точек к координатам окна
    for (short i = 0; i < 4; i++) {
        points[i] = this->toWindowCoords(points[i]);
    }
...
void CoordinateScene::createHouse()
{
    this->removeHouse();
    house = new House(this);
    for (QGraphicsItem * item : house->objects) {
        this->addItem(item);
    }
    this->update();
}

void CoordinateScene::removeHouse()
{
    if (house != nullptr) {
        delete house;
        house = nullptr;
